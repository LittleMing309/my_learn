        ...
        <div class="account-address">
            <div class="gray-box clear">
                <div class="title pre-title">
                    ...
                    <span class="gray-blue-btn js-add-address" @click="showPop"><a class="add">添加新地址</a></span>
                </div>
                <div class="box-inner clear">
                    <div class="address-list-item default-item" :data-id="receive.receiveId" v-for="(receive,index) in receiveInfo" :key="index">
                        ...
                        <div class="operation fn-right">
                            <div class="operation-cell">
                                <a class="gray-edit-btn js-edit-address" @click="editReceiveHandle(receive.receiveId)">修改</a>
                                <span class="red-del-btn js-del-address" @click="delReceiveHandle(receive.receiveId)"> 
                                    <a>删除</a>
                                    <em>删除</em> 
                                </span>
                            </div>
                        </div>
                        <div class="default fn-right" v-if="receive.default">  （默认地址）  </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState(['receiveInfo','editReceiveInfo'])
        },
        components: {
            addressPop
        },
        methods: {
            ...
            closePop(){
                this.popShow = false
                this.$store.commit('removeEditReceive')
            },
            ...mapMutations({
                delReceiveHandle: 'delReceive',
                // : 'editReceive'
            }),
            editReceiveHandle(id){
                this.$store.commit('editReceive',id)
                this.popShow = true
            }
        }
    }
