            ...
            <div class="item-box">
                <div class="gallery-wrapper">
                    <div class="gallery">
                        <div class="thumbnail">
                            <ul>
                                <li :class="{'on':index == imgIndex}" @click="tableImg(index)" v-for="(img,index) in itemsInfo.ali_images" :key="index"><img :src="img+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
                            </ul>
                        </div>
                        <div class="thumb">
                            <ul>
                                <li :class="{'on':index == imgIndex}" v-for="(img,index) in itemsInfo.ali_images" :key="index"><img :src="img+'?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'"></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="banner">
                    ...
                    <div class="sku-dynamic-params-panel">
                        <div class="sku-dynamic-params clear">
                            ...
                            <ul class="params-colors">
                                <li :class="{'cur': color.id == $route.query.itemId }" v-for="(color,index) in itemsInfo.sku_list" :key="index">
                                    <router-link :title="color.color" :to="{name:'Item',query:{itemId:color.id}}"><img :src="'http://img01.smartisanos.cn/'+color.image+'20X20.jpg'"></router-link>
                    ...
                    <div class="sku-status">
                        <div class="cart-operation-wrapper clearfix">
                            ...
                            <span class="gray-title-btn" @click="buyNowHandle()">
                                <a>现在购买</a>
                            </span>
                            <!-- <router-link class="gray-title-btn" tag="span" to="/checkout"><a>现在购买</a></router-link> -->
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        addCarPanelHandle(){
            let data = {info:this.itemsInfo, count: this.count}
        ...
        buyNowHandle(){
            let data = this.itemsInfo
            Vue.set(data,'count',this.count)
            this.$store.commit('buyNow',data)
