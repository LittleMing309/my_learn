                                ...
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                        <select class="city select-city js-form-city js-verify" v-model="receive.cityId">
                                            ...
                                            <option :value="city.area_id" v-for="(city,index) in cityList" :key="index">
                                    ...
                                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                        <select class="city select-city js-form-city js-verify" v-model="receive.countyId">
                                            ...
                                            <option :value="county.area_id" v-for="(county,index) in countyList" :key="index">{{county.area_name}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="module-form-row">
...
<script>
    ...
    export default {
        ...
        created() {
            if(this.receiveInfo.receiveId!=undefined){
                console.log(this.receiveInfo)
                this.receive = this.receiveInfo
            }
        },
        watch: {
            'receive.countyId' (){
        ...
        computed: {
            ...
            // },
            receiveInfo(){
                return this.$store.state.editReceiveInfo
            },
            cityList () {
                let cityList = []
        ...
        methods: {
            ...
            inspectReceive () {
                if(!this.phoneError && this.receive.name && this.receive.province && this.receive.city && this.receive.county && this.receive.add){
                    this.isRight = true
                }else{
                    this.isRight = false
                }
            ...
            submitReceiveHandle () {
                if(this.receiveInfo.edit){
                    this.$store.commit('submitEditReceive',this.receive)
                    this.$emit('close')
                }else{
                    this.receive.receiveId = new Date().getTime()
